import { createMove, evaluateBoard, makeMove, updateScores } from ".";
import { getNextPlayer } from "../utils/getNextPlayer";

import type { PlayHumanTurnOptions, TurnResult } from "./types";
...
export function playHumanTurn({
  ...
  // Apply player's move
  const nextBoard = makeMove(board, index, player);
